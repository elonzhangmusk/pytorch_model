...
if __name__ == '__main__':
    ...
    net = torch.nn.DataParallel(net, device_ids=[0,1,2,3])  # 四个 GPU
    net = net.to(device)
    TCMEyes_training_loader = get_training_dataloader_TCMEyes(
        mean=settings.LEFTEYE_TRAIN_MEAN,
        std=settings.LEFTEYE_TRAIN_STD,
        num_workers=4,
        batch_size=args.b,
        shuffle=True
)
    # TCMEyes_training_loader = get_training_dataloader(
    ...
    epoches = int(args.num_iter / len(TCMEyes_training_loader)) + 1
